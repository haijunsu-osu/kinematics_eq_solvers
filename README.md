...
2. **Single-Angle Trigonometric System**: $A \left[ \begin{array}{c} \cos \theta \\ \sin \theta \end{array} \right] = \mathbf{c}$
3. **Two-Angle Trigonometric System**: $A \left[ \begin{array}{c} \cos \theta_1 \\ \sin \theta_1 \end{array} \right] + B \left[ \begin{array}{c} \cos \theta_2 \\ \sin \theta_2 \end{array} \right] = \mathbf{c}$
...
- `prompts/`: Text files containing the prompts used to generate the code via Codex 5.1.
  ...
  - `prompt_solve_bilinear_sys.txt`
- `eq_solvers.tex`: LaTeX source for the accompanying paper.
- `eq_solvers.pdf`: Compiled PDF of the paper (if available).
